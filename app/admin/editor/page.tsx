...
const inputCls =
  "w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-sm text-gray-200 font-mono outline-none focus:border-amber-700 transition-colors";
const btnCls =
  "px-3 py-1.5 rounded border border-gray-700 text-xs font-mono text-gray-500 hover:text-gray-300 transition-all";
const btnPrimary =
  "px-4 py-1.5 rounded border border-amber-700/60 text-xs font-mono text-amber-400 hover:bg-amber-900/20 transition-all disabled:opacity-50";
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="block text-[11px] font-mono text-gray-600 mb-1">{label}</label>
      {children}
    </div>
  );
}
...
function NewNovelModal({ onClose, onCreated }: { onClose: () => void; onCreated: (n: Novel) => void }) {
  ...
  const autoSlug = (title: string) =>
    title.toLowerCase().replace(/[\s　]+/g, "-").replace(/[^\w-]/g, "").slice(0, 60);
    ...
    <Modal title="新しい小説を作成" onClose={onClose}>
      <Field label="タイトル *">
        <input className={inputCls} value={form.title}
          onChange={(e) => { const t = e.target.value; setForm((f) => ({ ...f, title: t, slug: f.slug || autoSlug(t) })); }}
          placeholder="影の縁で" />
      </Field>
      <Field label="スラッグ *">
        <input className={inputCls} value={form.slug}
          onChange={(e) => setForm((f) => ({ ...f, slug: e.target.value }))}
          placeholder="kage-no-fuchi-de" />
      </Field>
      <Field label="著者 *">
        <input className={inputCls} value={form.author}
          onChange={(e) => setForm((f) => ({ ...f, author: e.target.value }))}
          placeholder="財団アーキビスト" />
      </Field>
      <Field label="あらすじ">
        <textarea className={inputCls + " h-20 resize-none"} value={form.summary}
          onChange={(e) => setForm((f) => ({ ...f, summary: e.target.value }))} />
      </Field>
      <Field label="分類">
        <select className={inputCls} value={form.classification}
          onChange={(e) => setForm((f) => ({ ...f, classification: e.target.value }))}>
          {["Narrative", "Report", "Interview", "Log"].map((c) => <option key={c}>{c}</option>)}
        </select>
      </Field>
      {error && <p className="text-red-400 text-xs font-mono">{error}</p>}
      <div className="flex justify-end gap-2 mt-4">
        <button onClick={onClose} className={btnCls}>キャンセル</button>
        <button onClick={submit} disabled={loading} className={btnPrimary}>{loading ? "作成中…" : "作成"}</button>
      </div>
    </Modal>
  );
}

function NewChapterModal({ novelId, nextNumber, onClose, onCreated }: {
  novelId: string; nextNumber: number; onClose: () => void; onCreated: (c: Chapter) => void;
}) {
  const [title, setTitle] = useState(`第${nextNumber}章：`);
  ...
  const submit = async () => {
    ...
    const res = await fetch("/api/admin/chapters", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ novelId, title, chapterNumber: num, content: "" }),
    });
    const data = await res.json();
    if (!res.ok) { setError(data.error || "エラー"); setLoading(false); return; }
    onCreated(data);
    ...
    <Modal title="新しい章を作成" onClose={onClose}>
      <Field label="章タイトル *">
        <input className={inputCls} value={title} autoFocus
          onChange={(e) => setTitle(e.target.value)} placeholder="第1章：…" />
      </Field>
      <Field label="章番号">
        <input className={inputCls} type="number" value={num} min={1}
...
// 本文字数（タグ・空白除外）を計算
function calcBodyChars(content: string) {
...
export default function EditorPage() {
  ...
  // ドラッグによる分割幅調整
  const [splitPct,     setSplitPct]     = useState(50); // エディタ側の幅%
  ...
  useEffect(() => {
    fetch("/api/admin/novels").then((r) => r.json()).then(setNovels).catch(console.error);
  }, []);

  useEffect(() => {
    if (!selectedNovel) return;
    fetch(`/api/admin/chapters?novelId=${selectedNovel.id}`)
      .then((r) => r.json()).then(setChapters).catch(console.error);
  }, [selectedNovel]);
  ...
  const save = useCallback(async (chapterId: string, titleToSave: string, contentToSave: string) => {
    ...
    try {
      ...
      isDirty.current = false;
      setChapters((prev) =>
        prev.map((c) => c.id === chapterId ? { ...c, title: titleToSave, content: contentToSave } : c)
      );
    } catch {
      setSaveState("error");
    }
  ...
  const handleContentChange = useCallback((v: string) => {
    setContent(v);
    scheduleAutosave(chapterTitle, v);
  }, [chapterTitle, scheduleAutosave]);
  ...
  const togglePublish = async () => {
    ...
    await fetch(`/api/admin/chapters/${selectedChapter.id}`, {
      method: "PATCH", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ status: newStatus }),
    });
    const updated = { ...selectedChapter, status: newStatus };
    setSelectedChapter(updated);
    setChapters((prev) => prev.map((c) => c.id === selectedChapter.id ? updated : c));
  };
          ...
          <div className="flex flex-col flex-1 overflow-hidden">
            <div className="p-2 border-b border-gray-800/50">
              <button onClick={() => setShowNewNovelModal(true)}
                className="w-full py-1.5 rounded border border-dashed border-gray-700 text-[11px] font-mono
              ...
              {novels.map((novel) => (
                <button key={novel.id}
                  ...
                  <div className="text-[10px] font-mono text-gray-700 mt-0.5">
                    ...
                    {novel.chapterCount !== undefined && ` · ${novel.chapterCount}章`}
                  </div>
                </button>
              ))}
            </div>
              ...
              <>
                <div className="px-3 py-2 border-b border-gray-800/50">
                  ...
                  <button onClick={() => setShowNewChapterModal(true)}
                    className="mt-1.5 w-full py-1.5 rounded border border-dashed border-gray-700 text-[11px] font-mono
                      text-gray-600 hover:text-gray-400 hover:border-gray-600 transition-all">
                    + 新しい章
                  </button>
                </div>
                <div className="flex-1 overflow-y-auto py-1">
                  {chapters.map((ch) => (
                    <button key={ch.id} onClick={() => setSelectedChapter(ch)}
                      className={`w-full text-left px-3 py-2.5 text-xs transition-colors
                        ${selectedChapter?.id === ch.id ? "bg-gray-800/60 text-gray-100" : "text-gray-500 hover:bg-gray-800/30 hover:text-gray-300"}`}>
                      ...
                      <div className="text-[10px] font-mono mt-0.5 ml-4 flex gap-2">
                        {ch.status === "published"
                          ? <span className="text-green-700">公開中</span>
                          : <span className="text-gray-700">下書き</span>}
                        {/* 本文字数（タグ除外）を表示 */}
                  ...
                  {chapters.length === 0 && (
                    <p className="px-3 py-4 text-[11px] text-gray-700 font-mono text-center">章がありません</p>
                  )}
                </div>
              </>
            ) : (
              <div className="flex-1 flex items-center justify-center">
                <p className="text-[11px] font-mono text-gray-700 text-center px-4">
                  「小説」タブから<br />小説を選んでください
                </p>
              </div>
            )}
          </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex-shrink-0 flex items-center gap-3 px-4 py-2 border-b border-gray-800 bg-[#080c10]">
          {selectedChapter ? (
            ...
            <span className="flex-1 text-gray-700 text-sm font-mono">
              {selectedNovel ? "章を選択してください" : "小説を選択してください"}
            </span>
          )}

          {selectedChapter && (
            <span className={`text-[11px] font-mono flex-shrink-0 ${SAVE_LABELS[saveState].color}`}>
          ...
          {selectedChapter && (
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <button
                ...
                className="text-[11px] font-mono px-3 py-1 rounded border border-amber-800/60 text-amber-500 hover:bg-amber-900/20 transition-all">
                保存
              </button>
              <button onClick={deleteChapter}
                className="text-[11px] font-mono px-2 py-1 rounded border border-red-900/40 text-red-700 hover:text-red-500 hover:border-red-700 transition-all"
                title="章を削除">✕</button>
            </div>
          )}
        </div>
        ...
        {/* エディタ/プレビューエリア */}
        {selectedChapter ? (
          <div ref={splitPaneRef} className="flex-1 flex min-h-0 overflow-hidden select-none">
            {/* エディタパネル */}
            {(panelMode === "editor" || panelMode === "split") && (
              <div
                className="flex flex-col min-h-0 overflow-hidden border-r border-gray-800"
                ...
                <div className="px-3 py-1.5 border-b border-gray-800/50 flex items-center">
                  ...
                  <span className="ml-auto text-[10px] font-mono text-gray-700">Ctrl+S で保存</span>
                </div>
                <div className="flex-1 min-h-0">
                  <NmlEditor value={content} onChange={handleContentChange}
                    placeholder={"ここに本文を書いてください...\n\n例: [ANOMALY|SCP-0048|記憶を食べる影]が現れた。"} />
                </div>
              </div>
            )}
            ...
            {panelMode === "split" && (
              <div
                ...
                className="w-1 flex-shrink-0 bg-gray-800 hover:bg-amber-700/50 cursor-col-resize transition-colors"
                title="ドラッグで幅を調整"
              />
            )}
            ...
            {(panelMode === "preview" || panelMode === "split") && (
              <div className="flex flex-col min-h-0 overflow-hidden flex-1">
                <div className="px-3 py-1.5 border-b border-gray-800/50 flex items-center">
                  ...
                  <span className="ml-auto text-[10px] font-mono text-gray-700">リアルタイム</span>
                </div>
          ...
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="font-mono text-gray-800 text-xs">または新しい章を作成してください</p>
              {selectedNovel && (
                <button onClick={() => setShowNewChapterModal(true)}
                  className="mt-5 px-4 py-2 rounded border border-dashed border-gray-700
                    text-sm font-mono text-gray-600 hover:text-gray-400 hover:border-gray-600 transition-all">
                  + 最初の章を作成
                </button>
              )}
            </div>
